...
Analyses <- emission_fusion %>% 
  ...
  bind_rows(concentration_pesticide_fusion)
...
polluant_et_pesticide <- emission_final %>% 
  ...
  bind_rows(concentration_pesticide_final)
...
```{r pressure, echo=FALSE, fig.cap="Probleme coordonnees DROM", out.width = '50%'}
knitr::include_graphics("doc_projet/problème_emplacement_drom.PNG")
```
...
all_SPDF <- cbind(all_SPDF@data,all_SPDF@coords) %>% mutate(Region = str_to_title(Region))
...
```{r pressure, echo=FALSE, fig.cap="Enrepot de données", out.width = '75%'}
...
Il est possible de selection la region à gauche, l'année et le type de polluant/pesticide
...
kable(df_id_pol, caption = "Tableau recapitulatif des résultat de test de Stone")
```
